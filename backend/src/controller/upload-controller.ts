...
export default class UploadController {
  ...
  handleARIS66xCsvUpload: RequestHandler = async (req, res, next) => {
    ...
    this.transferService
      .createTransferMetaDataV2(new CSVDataExtractor(), uploadedFile.buffer)
      .then(async (result) => {
        try {
  ...
  handleARIS66xUpload: RequestHandler = async (req, res, next) => {
    try {
      ...
      const newTransfer = await this.transferService.createTransferMetaData(
        uploadedFile.path
      );
      const newTransferId = newTransfer?._id || new mongoose.mongo.ObjectId(0);
      const hashDigitalFileList =
        await this.digitalFileListService.createDigitalFileListMetaData(
          newTransferId.toString(),
          uploadedFile.path
        );

      this.documentationPath = await this.s3ClientService.uploadAra66xFile(
        uploadedFile
      );
  ...
  get66xFileTransferInfos: RequestHandler = async (req, res, next) => {
    try {
      ...
      const transferData = await this.transferService.extractsTransferInfo(
        aris66xFile.path
      );
  ...
  handleARIS617Upload: RequestHandler = async (req, res, next) => {
    try {
      ...
      this.documentationPath = await this.s3ClientService.uploadARIS617File(
        uploadedFile,
        this.documentationPath
      );
  ...
  saveFolderDetails: RequestHandler = async (req, res, next) => {
    try {
      ...
      const {
        ...
        transferId,
        applicationNumber,
        accessNumber: accessionNumber,
        classification: primarySecondary,
        technicalV2: techMetadatav2,
        note: note,
      } = body;
      ...
      await this.fileService.saveFolderDetails(
        file,
        receivedChecksum,
        transferId,
        applicationNumber,
      ...
      res
        .status(500)
        .json({ error: "An error occurred in the saveFolderDetails Function" });
    }
  };
  ...
  downloadUpdateAris662: RequestHandler = async (req, res, next) => {
    ...
    try {
      ...
      const updatedExcelBuffer = await this.fileService.downloadUpdateAris662(
        accessionNumber,
        applicationNumber
      );
      res.setHeader(
        "Content-Disposition",
        `attachment; filename=Update_Aris662-${accessionNumber}_${applicationNumber}.xlsx`
      );
      res.setHeader(
        "Content-Type",
  ...
  saveToS3Documentation: RequestHandler = async (req, res, next) => {
    ...
    try {
      ...
      await this.s3ClientService.saveToS3Documentation(
        file.buffer,
        fileName,
        subApplicationPath
      );
