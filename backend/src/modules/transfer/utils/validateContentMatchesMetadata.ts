...
export const validateContentMatchesMetadata = async ({
	...
	const requiredDirectories = new Set(["content"]);
	...
	const folderNames = folderPaths.map((folderPath) => {
		...
		return folderParts[folderParts.length - 1]; // Get the folder name from the path
	});
	...
	await new Promise<void>((resolve, reject) => {
		yauzl.fromBuffer(buffer, { lazyEntries: true }, (err, zipFile) => {
			...
			zipFile.on("entry", (entry) => {
				...
				if (parentDirectory === "content" && folderNames.includes(parts[1])) {
					foundContentEntries.add(parts[1]);
				}
