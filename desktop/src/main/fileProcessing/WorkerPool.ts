  ...
  runTask<T, U>(
    ...
    return new Promise<U>((resolve, reject) => {
      ...
      worker.on("message", (message) => {
        ...
        // Handle invalid path updates
        else if (message.type === "missingPath") {
          console.log(`Missing path ${message.path} message emitted.`);
        ...
        // Handle unexpected messages
          ...
          if (timer) clearTimeout(timer);
          reject(
            new Error(`Unexpected message type "${message.type}" from worker.`)
          );
        }
      });
