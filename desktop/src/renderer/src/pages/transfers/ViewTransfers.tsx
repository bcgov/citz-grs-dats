...
export const ViewTransfersPage = () => {
	...
	const [showConfirmReDownloadModal, setShowConfirmReDownloadModal] = useState(false);

	const [downloadAccession, setDownloadAccession] = useState<string | null>(null);
	const [downloadApplication, setDownloadApplication] = useState<string | null>(null);
	...
	useEffect(() => {
		if (preserveAccession && preserveApplication) handlePreserveTransferRequest();
	...
	const handleFetchTransfers = async () => {
		...
		try {
			...
			if (jsonResponse.success) {
				// Set transfers
				const fetchedTransfers = jsonResponse.data.transfers?.map((t, index) => {
					return {
						id: index,
	...
	const handleDeleteTransferRequest = async () => {
		...
		try {
			...
			if (jsonResponse.success) {
				...
				setTransfers((prev) => {
					return prev.map((t) => {
						if (t.accession === deleteAccession && t.application === deleteApplication) {
			...
			if (jsonResponse.success) {
				...
				setTransfers((prev) => {
					const updatedTransfers = prev.map((t) => {
						if (t.accession === downloadAccession && t.application === downloadApplication) {
							// Make edit to status
							return {
								...t,
								status: t.status === "Preserved" ? "Downloaded & Preserved" : "Downloaded",
							};
						}
	...
	const handlePreserveTransferRequest = async () => {
		...
		try {
			...
			if (jsonResponse.success) {
				...
				setTransfers((prev) => {
					const updatedTransfers = prev.map((t) => {
						if (t.accession === preserveAccession && t.application === preserveApplication) {
							// Make edit to status
							return {
								...t,
								status: t.status === "Downloaded" ? "Downloaded & Preserved" : "Preserved",
							};
						}
						return t; // Return unchanged
					});
	...
	window.electron.ipcRenderer.on("download-transfer-complete", (_event, filePath) => {
		if (downloadSuccess !== true) {
	...
	const handleSearch = () => {
		...
		if (deletedTransfersOnly)
			setFilteredTransfers(filtered.filter((t) => t.status === "Transfer deleted"));
		else setFilteredTransfers(filtered.filter((t) => t.status !== "Transfer deleted"));
	};

	useEffect(() => {
		if (deletedTransfersOnly)
			setFilteredTransfers(transfers.filter((t) => t.status === "Transfer deleted"));
		else setFilteredTransfers(transfers.filter((t) => t.status !== "Transfer deleted"));
			...
			<Grid size={8} sx={{ paddingTop: 3 }}>
				<Stack gap={2}>
					...
					>
						...
						<Switch isSelected={deletedTransfersOnly} onChange={setDeletedTransfersOnly}>
