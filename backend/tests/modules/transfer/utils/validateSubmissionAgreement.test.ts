...
describe("validateSubmissionAgreement", () => {
  ...
  it("should validate the PDF successfully", async () => {
    ...
    const mockPDF = createMockPDF(1, [
      `Accession # ${accession} and Application # ${application}`,
    ]);
    (pdfjsDist.getDocument as jest.Mock).mockReturnValue(mockPDF);
  ...
  it("should throw an error for invalid PDF structure", async () => {
    ...
    (pdfjsDist.getDocument as jest.Mock).mockImplementation(() => ({
  ...
  it("should handle unexpected errors gracefully", async () => {
    ...
    await expect(
      ...
      new HttpError(
        500,
        "An error occurred while validating the PDF: Unexpected error"
      )
    );
  });
  ...
  it("should throw a generic error for non-Error objects", async () => {
    ...
    await expect(
      ...
      new HttpError(500, "An unknown error occurred while validating the PDF.")
    );
  });
});
