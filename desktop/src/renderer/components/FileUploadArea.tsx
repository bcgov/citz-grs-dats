...
export const FileUploadArea = ({ file, accept, onChange, onDrop, onDelete }: Props) => {
	...
	const handleDrop = (e: React.DragEvent<HTMLDivElement>) => {
		...
		if (e.dataTransfer.files && e.dataTransfer.files.length > 0) {
			...
			const isAccepted = acceptedTypes.some((type) => {
				return (
					droppedFile.type === type || (type.startsWith(".") && droppedFile.name.endsWith(type))
				);
			});
	...
	return (
		<Box
			...
			height={"15rem"}
			border={"solid 1px"}
			onClick={() => !isInvalidFile && document.getElementById("file-input")!.click()}
			onDrop={handleDrop}
				...
				{file ? (
					...
					>
						...
						<Stack gap={"4px"}>
							...
							<Typography sx={{ fontSize: "0.8em" }}>{formatFileSize(file.size)}</Typography>
						</Stack>
						<Box
							sx={{
								display: "flex",
					...
					>
						<UploadFileIcon />
						<Typography sx={{ fontSize: "16px" }}>Choose a file or drag and drop here</Typography>
						<Typography
							sx={{
								padding: "8px 16px",
								fontSize: "16px",
