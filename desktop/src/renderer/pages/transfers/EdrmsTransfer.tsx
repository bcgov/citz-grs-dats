	...
	const handleSendRequest = async () => {
		...
		for (let index = 0; index < totalChunks; index++) {
			...
			try {
				...
				notify.log(`Uploading chunk ${index + 1} of ${totalChunks}`);
				...
				if (!response.ok) {
					notify.error(`Upload failed for chunk ${index + 1}`);
					setRequestSuccessful(false);
					return;
				}

				const jsonResponse = await response.json();
				notify.log(`EDRMS transfer response: ${jsonResponse}`);

				if (jsonResponse.success && index === totalChunks - 1) {
					setRequestSuccessful(true);
				}
			} catch (error) {
				notify.error(`EDRMS transfer error: ${error}`);
				setRequestSuccessful(false);
				return;
			}
		}

		notify.log("All EDRMS chunks uploaded successfully.");
	};
