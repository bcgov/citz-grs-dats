...
export const LanConfirmationView = ({
	...
	const onFolderDelete = (folder: string) => {
		...
		setChanges((prev) => {
			const existingItemWithNewPath = prev.find((c) => c.newFolderPath === folder);
			const existingItemWithOriginalPath = prev.find((c) => c.originalFolderPath === folder);
		...
		notify.log(`Deleted folder: ${folder}`);
	};

	const disableNext =
		!folders.every((folder) => folder.metadataProgress + folder.bufferProgress === 200) ||
		folders.length === 0;
	...
	return (
		<Stack gap={3}>
			<Stack gap={3}>
				...
				<Typography>
					Please wait while DATS automatically loads the folders associated with your transfer. Once
					all folders are successfully loaded, verify the status before proceeding to the next step.
					If any folders are part of a current audit, FOI request, or legal case please remove them
					from the list before proceeding.
				</Typography>
			...
			<Box sx={{ display: "flex", justifyContent: "space-between" }}>
				...
				<Button onPress={onNextPress} isDisabled={disableNext} style={{ width: "fit-content" }}>
					Next
				</Button>
			</Box>
		</Stack>
